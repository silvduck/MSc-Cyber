...
def big_endian_to_little_endian(transaction):
    return unhexlify(transaction)[::-1]


# Function - Calculate Merkle root
def merkle_root_func(transactions_set):
    if len(transactions_set) != 1:
        # si el nombre de txs es senar duplicar la ultima tx
        if (len(transactions_set) % 2) != 0:
            transactions_set.append(transactions_set[-1])
        new_tx_set = []
        for i in range(0, len(transactions_set) - 1, 2):
            # calcular little endian de les txs
            rev_tx1 = big_endian_to_little_endian(transactions_set[i])
            rev_tx2 = big_endian_to_little_endian(transactions_set[i + 1])
            # concatenar les txs
            tx1_tx2 = rev_tx1 + rev_tx2
            # calcular doble sha256
            double_sha = sha256(sha256(tx1_tx2).digest()).digest()
            new_tx_set.append(hexlify(double_sha[::-1]))
        # repetir fins tenir un unic hash
        return merkle_root_func(new_tx_set)
    else:
        merkle_root = transactions_set[0].decode("utf-8")
        return merkle_root
